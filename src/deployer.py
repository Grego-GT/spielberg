...
class ActorDeployer:
    ...
    async def _create_actor(self, actor_files: Dict[str, str], actor_name: str) -> Dict[str, Any]:
        ...
        # Get and validate description (max 300 chars for Apify)
        description = actor_json.get('description', 'Actor generated by Spielberg')
        if len(description) > 300:
            Actor.log.warning(f"Description too long ({len(description)} chars), truncating to 300 chars")
        ...
        actor_data = {
            ...
            'is_public': False,
            'seo_title': actor_json.get('title', actor_name)[:60],  # SEO title max 60 chars
            'seo_description': description[:150],  # SEO description max 150 chars
            'versions': [{
                'versionNumber': '0.1',
